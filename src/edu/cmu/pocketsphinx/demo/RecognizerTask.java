...
public class RecognizerTask implements Runnable {
	 ...
	 * 
	 * This class implements a task which pulls blocks of audio from the
	 * system audio input and places them on a queue.
	 * 
	 * @author David Huggins-Daines <dhuggins@cs.cmu.edu>
	 */
	...
	public RecognizerTask() {
		pocketsphinx.setLogfile("/sdcard/data/zuoshu/pocketsphinx.log");
		Config c = new Config();
		/*
		 * In 2.2 and above we can use getExternalFilesDir() or
		 * whatever it's called
		 */
		c.setString("-hmm", "/sdcard/data/test/hmm/tdt_sc_8k");
		c.setString("-dict", "/sdcard/data/test/lm/test.dic");
		c.setString("-lm", "/sdcard/data/test/lm/test.lm");
	...
	public void run() {
		...
		while (!done) {
			...
			synchronized (this.mailbox) {
				todo = this.mailbox;
				/*
				 * If we're idle then wait for something
				 * to happen.
				 */
				...
				/*
				 * Reset the mailbox before releasing,
			...
			/*
			 * Do whatever's appropriate for the current
			 * state. Actually this just means processing
			 * audio if possible.
			 */
